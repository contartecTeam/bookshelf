...
const Relation = RelationBase.extend(
  /** @lends Relation.prototype */ {
     ...
     *
     * @param {string} attribute Can be any `attribute`
     * @param {Model} model The model obtained from the load fetch call.
     * @return {string|undefined}
     */
    getAttributeFormatted(attribute, model) {
      const formattedObject = model.parse({[attribute]: ''});

      let attrFormatted = null;

      if (formattedObject && Object.keys(formattedObject).length > 0) attrFormatted = Object.keys(formattedObject)[0];
      ...
      const grouped = _.groupBy(related, (m) => {
        ...
        if (m.pivot) {
          if (this.isInverse() && this.isThrough()) {
            ...
            key = m.pivot.get(this.getAttributeFormatted(this.key('foreignKey'), m));
          }
        } else if (this.isInverse()) {
          key = this.isForeignKeyTargeted() ? m.get(this.foreignKeyTarget) : m.id;
        } else {
          key = m.get(this.getAttributeFormatted(this.key('foreignKey'), m));
        }
        return idKey(key);
      });
